...
export default function EditUsernameForm(props) {
    ...
    const handleEditUsername = () => {
        console.log(props)
        const {value: newUsername} = document.getElementById("username")
        axios.put(`http://localhost:8080/edit-username/${props.id}`, { newUsername }).then(res => {
        const { data } = res
            document.getElementById("msg").innerText = data
            if(newUsername.trim() !== "") props.handleEditUsername(newUsername)
      }).catch(err => {
        console.log(err)
      }) 
    }
            ...
            <div className="w-full max-w-xs">
              <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-7">
                <div className="flex flex-col items-center justify-between">
                  <h3 className="text-xl mb-3">Edit Username</h3>
                </div>
                <div className="mb-6">
                  <label
